function toggleAccordion(button) {
  const content = button.nextElementSibling; // Это .accordion__panel
  const accordionContent = button.parentElement; // <-- ПОЛУЧАЕМ .accordion__content
  ...
  if (isOpened) {
    ...
    accordionContent.classList.remove("is-active"); // <-- УДАЛЯЕМ КЛАСС ЗДЕСЬ
    ...
    accordionContent.classList.add("is-active"); // <-- ДОБАВЛЯЕМ КЛАСС ЗДЕСЬ
